...
func TestClaudeCodeProvider_MockIntegration(t *testing.T) {
	...
	mockProvider := provider.NewMockModelProvider("local", []provider.Info{
		{
			ID:       "claude-code",
			Name:     "claude-code",
			Provider: "local",
	...
	response, usage, err := mockProvider.Generate(provider.GenerateContext{Context: ctx}, request, nil)
...
func TestClaudeCodeProvider_RealIntegration(t *testing.T) {
	...
	pr, err := NewClaudeCodeProvider(config)
	if err != nil {
...
func TestClaudeCodeProvider_WorkingDirectory(t *testing.T) {
	...
	assert.Equal(t, tempDir, pr.workingDir)
	...
	pr.workingDir = fullPath
	assert.Equal(t, fullPath, pr.workingDir)
}

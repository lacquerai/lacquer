...
func NewMCPServer(config *ast.MCPServerConfig) *MCPServer {
	return &MCPServer{
		config:    config,
...
func (s *MCPServer) initializeLocal(ctx context.Context) error {
	// Create the command
...
func (s *MCPServer) DiscoverTools(ctx context.Context) ([]MCPTool, error) {
	s.mu.RLock()
	if s.closed {
		s.mu.RUnlock()
...
func (s *MCPServer) ExecuteTool(ctx context.Context, toolName string, parameters json.RawMessage) (map[string]interface{}, error) {
	s.mu.RLock()
	if s.closed {
		s.mu.RUnlock()
		return nil, fmt.Errorf("server is closed")

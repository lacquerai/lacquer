...
type MCPToolProvider struct {
	servers map[string]*MCPServer // Map of tool name to MCP server
	mu      sync.RWMutex
}
...
func (p *MCPToolProvider) AddToolDefinition(tool *ast.Tool) ([]Tool, error) {
...
func (p *MCPToolProvider) ExecuteTool(ctx context.Context, toolName string, parameters json.RawMessage, execCtx *ToolExecutionContext) (*ToolResult, error) {
	p.mu.RLock()
...
func (p *MCPToolProvider) getOrCreateServer(tool *ast.Tool) (*MCPServer, error) {
